...
pub fn gen_array<T: rand::distributions::uniform::SampleUniform + PartialOrd + Copy>(
    length: usize,
    min: T,
    max: T,
) -> Vec<T> {
...
pub fn insertion_sort<T: Ord + Copy>(
    array: &mut [T],
    left: isize,
    right: isize,
    _: Option<Pivot>,
...
pub fn partition_hoare<T: PartialOrd + Copy>(
    array: &mut [T],
    lo: isize,
    hi: isize,
    p: Pivot,
...
pub fn quick_sort_lomuto<T: Ord + Copy>(
    array: &mut [T],
    lo: isize,
    hi: isize,
    pivot_type: Option<Pivot>,
) -> &mut [T] {
    if lo < hi {
        if hi - lo < 27 {
...
pub fn quick_sort_hoare<T: Ord + Copy>(
    array: &mut [T],
    lo: isize,
    hi: isize,
    pivot_type: Option<Pivot>,
...
pub fn counting_sort<T: Integer + NumCast + Ord + Copy + AddAssign + SubAssign>(
    array: &mut [T],
) -> &mut [T] {
    let n = array.len();
    ...
    let mut count = Vec::<T>::with_capacity(NumCast::from(range).unwrap());
    let mut output = Vec::<T>::with_capacity(n);
    for i in 0..NumCast::from(range).unwrap() {
        count.push(T::zero());
    }
    for i in 0..n {
        output.push(T::zero());
    ...
    for i in (0..n).rev() {
        let ind: usize = NumCast::from(array[i] - min).unwrap();
        let index: usize = NumCast::from(count[ind] - T::one()).unwrap();
